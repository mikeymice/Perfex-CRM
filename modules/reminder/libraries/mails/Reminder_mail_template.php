...
class Reminder_mail_template
{
     ...
     * @param array $fields
     */
    public function set_reminder_merge_fields($fields, ...$params)
    {
        $this->ci->load->library('reminder/merge_fields/app_reminder_merge_fields');
        if (!is_array($fields)) {
            $fields = $this->ci->app_reminder_merge_fields->format_feature($fields, ...$params);
        }

        $this->merge_fields = array_merge($this->merge_fields, $fields);
        // print_r($this->merge_fields);
        // die("==");
        return $this;
    }
    /**
